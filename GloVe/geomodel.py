...
class Model:
    ...
    def loadEmbeddingMatrix(self,empadding_wor_len,typeToLoad):
        from twgeo.data.twus_dataset import _load_data,load_state_data
        new_train_df, dev_df, test_df, df_concat = _load_data()
        x_train, y_train, x_test, y_test, x_dev, y_dev = load_state_data()
        embed_size=0
        train = new_train_df
        new_train_df['Preproceesed'] =  new_train_df['Preproceesed'].astype(str)
        list_sentences_train =  new_train_df['Preproceesed']
        ...
        if(typeToLoad=="glove"):
            path="/content/drive/MyDrive/glove.6B.50d.txt".format(empadding_wor_len)
            EMBEDDING_FILE =path
    ...
    def build_model(self, num_outputs, time_steps=500, vocab_size=20000, hidden_layer_size=128):
        ...
        self._model.add(Embedding(self._vocab_size,200 ))
    ...
    def train(self, x_train, y_train, x_dev, y_dev, epochs=20, reset_model=False):
        """
        ...
        if x_dev.shape[0] != y_dev.shape[0]:
            ...
            #checkpoint_path = "/content/drive/MyDrive/Colab Notebooks/Training models/paper_agg"
            #checkpoint_dir = os.path.dirname(checkpoint_path)
            #cp_callback =tf.keras.callbacks.ModelCheckpoint(checkpoint_path,save_best_only=True,save_weights_only=True,verbose=1,period=5)
            if self._use_tensorboard:
                callbacks = self._generate_callbacks()
            else:
                callbacks = []
            ...
            history = self._model.fit(x_train, y_train, epochs=epochs, batch_size=self._batch_size,
                                      validation_data=(x_dev, y_dev))#, callbacks=[cp_callback])
